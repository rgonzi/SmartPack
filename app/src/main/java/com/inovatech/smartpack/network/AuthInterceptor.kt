...
/**
 * Classe Interceptor que afegeix el token dins del Header per fer les peticions d'aquelles
...
class AuthInterceptor @Inject constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return if (token != null && shouldAddAuthorizationHeader(originRequest)) {
            val newRequest =
                originRequest.newBuilder().addHeader("Authorization", "Bearer $token").build()
     ...
     */
    private fun shouldAddAuthorizationHeader(request: Request): Boolean {
